...
export function ProcessStep({
    ...
    return (
        <div className="group relative text-center">
            {/* Connector line */}
            {!isLast && (
                <div className="absolute left-1/2 top-12 hidden h-0.5 w-full bg-white/10 lg:block" />
            )}

            {/* Step number */}
            <div className="relative mx-auto mb-6 flex h-24 w-24 items-center justify-center">
                {/* Outer ring */}
                <div className="absolute inset-0 rounded-full border-2 border-white/20 transition-all duration-500 group-hover:border-brand-green group-hover:scale-110" />
                {/* Inner circle */}
            ...
            <p className="text-sm text-concrete">{description}</p>
        </div>
    );
}
